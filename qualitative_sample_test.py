from qualitative_sample import QuantitativeSample
...
def filterPunctation(items: list) -> list:
    return list(
        filter(
...
QS = QuantitativeSample(filteredValues1)
QSen = QuantitativeSample(filteredValues2)

data1 = dict(QS.counted)
data2 = dict(QSen.counted)
...
ax1.grid()
ax2.grid()
ax1.autoscale(enable=False, axis='both', tight=False)
